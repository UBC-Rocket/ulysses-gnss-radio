 ...
 *
 * This is ~80x more efficient than per-byte interrupts for typical NMEA
 * traffic.
 *
...
#define GPS_NMEA_MAX                                                           \
  87u /**< Max NMEA sentence length (82 chars + safety margin) */
...
void gps_set_queue(gps_sample_queue_t *queue) { s_gps_queue = queue; }

void gps_set_fix_queue(gps_fix_queue_t *queue) { s_gps_fix_queue = queue; }

void gps_set_protocol_mode(spi_protocol_mode_t mode) { s_protocol_mode = mode; }
...
void gps_rx_event_callback(UART_HandleTypeDef *huart, uint16_t Size) {
  ...
  // Process exactly the bytes received in this chunk
...
static void populate_gps_fix(gps_fix_t *fix, const lwgps_t *gps) {
  ...
  fix->time_of_week_ms =
      ((uint32_t)gps->hours * 3600 + (uint32_t)gps->minutes * 60 +
